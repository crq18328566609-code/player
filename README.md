...
> 当系统提示中出现【飞行棋事件 · 已自动掷完】时：视为骰子已掷完、格子已生效，直接按格子内容完成，禁止再提出掷骰或编写掷骰代码。
...
- **接受方**：女仆版/SM版 → 无论谁停都是女方接受；男仆版 → 男方接受；其他版本 → 谁停谁接受。
- **互相/一起/双方**：按字面双方一起完成，不要理解成「两个人各受一次惩罚」。
- **浮窗**：最小化胶囊须 `inline-flex` + `height:auto`，避免被全局 `button{flex:1}` 拉成竖条。
- **后退格**：只有停在该格才生效，路过不触发。
